...
from bin.cp import _home_relative_path, main
...
def test_main_multiple_sources_to_non_directory_dest(tmp_path, capsys):
    ...
    assert (
        f"cp: target '{_home_relative_path(dest_file)}' is not a directory"
        in captured.err
    )
...
def test_main_non_existent_source(tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert (
        f"cp: {_home_relative_path(non_existent)}: No such file or directory"
        in captured.err
    )
...
def test_main_copy_file_onto_itself(monkeypatch, tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert (
        f"cp: {_home_relative_path(source_file)}: '{source_file}' and '{source_file}' are the same file"
        in captured.err
    )
...
def test_main_shutil_error(monkeypatch, tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert (
        f"cp: {_home_relative_path(source_file)}: Simulated shutil error"
        in captured.err
    )
