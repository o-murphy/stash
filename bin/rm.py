...
def rm(path: Path, args):
    ...
    if path.is_dir():
        ...
        if (
            args.interactive
            and not args.force
            and input(f"rm: descend into directory '{path}'? [y/N] ").lower() != "y"
        ):
            return
    ...
    if path.is_file():
        if (
            args.interactive
            and not args.force
            and input(f"rm: remove regular file '{path}'? [y/N] ").lower() != "y"
        ):
            return
...
def main(args):
    ...
    ap.add_argument(
        "paths", type=Path, nargs="+", help="files or directories to delete"
    )
