...
from .tree_format import CHARSETS, fmt_error, fmt_path
...
def filter_tree(tree_dict, ns):
    ...
    for item, content in tree_dict.items():
        ...
        if ns.I and should_match_pattern and check_pattern(name, ns.I, ns.ignore_case):
            continue
        if (
            ns.P
            and should_match_pattern
            and not check_pattern(name, ns.P, ns.ignore_case)
        ):
            continue
